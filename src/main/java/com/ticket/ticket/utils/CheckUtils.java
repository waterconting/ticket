...
public class CheckUtils {
    ...
    /* 手机号验证
   regex1验证条件很方便，可以大范围覆盖，但是还是细分一下使用regex2验证条件比较好一些。*/
    ...
    public static boolean isCarNo(String carNo) {
        ...
        if (carNo.length()==7){
            //普通车辆
             p = Pattern.compile("^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}(?:(?![A-Z]{4})[A-Z0-9]){4}[A-Z0-9挂学警港澳]{1}$");
        }else if (carNo.length()==8) {
            //新能源车辆
            p = Pattern.compile("^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}(([0-9]{5}[DF]$)|([DF][A-HJ-NP-Z0-9][0-9]{4}$))");
        }else {
            return false;
        }
